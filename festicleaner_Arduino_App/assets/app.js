...
function initSocketIO() {
    ...
    // --- CATCH THERMO DATA FROM PYTHON ---
    socket.on('thermo_data', (data) => {
        // Because Python sends temp and humidity in two separate messages, we check which one arrived
        ...
        if (data.humidity !== undefined) {
            const humElement = document.getElementById('humVal');
            if (humElement) humElement.innerText = data.humidity + " %";
        }
    });
    // ------------------------------------------
